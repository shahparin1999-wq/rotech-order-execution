...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ expectedShipments: snapshot.state.expectedShipments, files: snapshot.state.shipmentFiles, parseRuns: snapshot.state.parseRuns, parsedFields: snapshot.state.parsedFields, version: snapshot.version });
  } catch (err) {
